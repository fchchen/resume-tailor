...
async function loadDataFile(filename) {
  // Security: Prevent path traversal by only allowing filenames within the data directory
  ...
  try {
    ...
    console.error(`Error loading file ${safeName}:`, err.message);
    // Fallback to defaults if specific file fails
    if (safeName.includes('resume')) return await fs.readFile(path.join(DATA_DIR, 'resume.md'), 'utf-8');
    if (safeName.includes('cover')) return await fs.readFile(path.join(DATA_DIR, 'cover-letter.md'), 'utf-8');
    throw new Error('Required data file not found.');
  }
}

async function buildTailorPrompt({ jobTitle, company, jobDescription, githubRepos, baseResumeFile, baseCoverLetterFile }) {
  const resume = await loadDataFile(baseResumeFile || 'resume.md');
  const coverLetter = await loadDataFile(baseCoverLetterFile || 'cover-letter.md');
