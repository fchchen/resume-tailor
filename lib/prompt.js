...
async function loadDataFile(filename) {
  try {
    ...
    // Try to load a generic file as fallback
    if (filename.includes('resume')) return await fs.readFile(path.join(DATA_DIR, 'resume.md'), 'utf-8');
    if (filename.includes('cover')) return await fs.readFile(path.join(DATA_DIR, 'cover-letter.md'), 'utf-8');
    throw err;
  }
}
