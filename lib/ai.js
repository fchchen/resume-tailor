...
function spawnWithStdin(command, args, prompt, envOverrides = {}) {
  return new Promise((resolve, reject) => {
    const env = { ...process.env, ...envOverrides };
    delete env.CLAUDECODE;
    delete env.CLAUDE_CODE_ENTRYPOINT;

    const child = spawn(command, args, { env, timeout: TIMEOUT_MS });
...
function callClaude(prompt) {
  // Pass an empty string to -p so it doesn't try to use standard prompt prefixes,
  // then pipe the actual prompt via stdin.
  return spawnWithStdin('claude', ['-p', '', '--output-format', 'text'], prompt);
}

function callGemini(prompt) {
  return spawnWithStdin('gemini', ['-p', '', '--output-format', 'text'], prompt);
}
